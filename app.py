...
def _clean_str(value):
    """
    Retorna string não-vazia ou None. Trata NaN (float), None, números e
    strings 'nan' que vêm do yfinance / pandas Series com dados faltantes.
    """
    if value is None:
        return None
    try:
...
def main():
    ...
    with st.sidebar:
        ...
        top_n = st.slider(
            "Quantidade de ativos no ranking", 5, 20, 10,
            key="sl_top_n",
        )
        ...
        run_btn = st.button(
            "Atualizar análise", type="primary", use_container_width=True,
            key="bt_atualizar",
        )
    ...
    if escolha:
        ...
        with st.expander("Sobre o ativo", expanded=True):
            # Todos os campos textuais passam por _clean_str para neutralizar
            # NaN/None/strings vazias que o yfinance pode devolver.
            descricao = _clean_str(linha.get("descrição"))
            nome_longo = _clean_str(linha.get("nome_longo"))
            nome_curto = _clean_str(linha.get("nome"))

            if nome_longo and nome_longo != nome_curto:
            ...
            metadados = []
            industry = _clean_str(linha.get("industry"))
            if industry:
                metadados.append(("Indústria", industry))

            country = _clean_str(linha.get("country"))
            city = _clean_str(linha.get("city"))
            state = _clean_str(linha.get("state"))
            partes = [p for p in [city, state, country] if p]
            if partes:
                metadados.append(("Sede", ", ".join(partes)))

            funcionarios = _to_float(linha.get("funcionarios"))
            if funcionarios is not None and funcionarios > 0:
                metadados.append(
                    ("Funcionários", f"{int(funcionarios):,}".replace(",", "."))
                )

            mc = _to_float(linha.get("market_cap"))
            if mc is not None and mc > 0:
                metadados.append(("Valor de mercado", format_money(mc)))

            website = _clean_str(linha.get("website"))
            if website:
